// source: https://www.freetype.org/freetype2/docs/tutorial/example2.cpp
// example2.cpp
...
ftcore_CreateFtLibrary(void)
{
	...
	FT_Error   ft_status  = 0;

	ft_status = FT_Init_FreeType(&ft_library);
	if(ft_status == 0)
	{
		FTLOG_INF("sucessfully created library instance: %p\n", ft_library);
	} else {
		FTLOG_ERR("unable to create library instance!\n");
	}
...
ftcore_DestroyFtLibrary(FT_Library *pft_library)
{
	...
	ft_result = FT_Done_FreeType(ft_library);
	if(ft_result == 0)
	{
		FTLOG_INF("destroyed FreeType library.\n");
	} else {
...
ftcore_CreateFace(FT_Library ft_library, const char *ft_fontpath, FT_Long ft_index)
{
	...
	FTLOG_INF("creating face using %s @index %d...\n", ft_fontpath, ft_index);
	ft_result = FT_New_Face(ft_library, ft_fontpath, ft_index, &ft_face);
	if(ft_result == 0)
	{
		FTLOG_INF("sucessfully created face: %p\n", ft_face);
	} else {
		FTLOG_ERR("unable to create face!\n");
	}
...
ftcore_DestroyFace(FT_Face *pft_face)
{
	...
	ft_result = FT_Done_Face(ft_face);
	if(ft_result == 0)
	{
		FTLOG_INF("sucessfully destroyed face.\n");
	} else {
